...
from t2m_runtime.utils import _init_globals, get_property, objectIs, to_js_obj, to_py_list, wait_promise

struct BashExecutorOptions(ImplicitlyCopyable):
    var onChunk: PythonObject
    var signal: PythonObject

    def __init__(out self, onChunk: PythonObject, signal: PythonObject):
        self.onChunk = onChunk
        self.signal = signal

    def __init__(out self, *, copy: Self):
        self.onChunk = copy.onChunk
        self.signal = copy.signal

    def __init__(out self, py: PythonObject) raises:
        self.onChunk = get_property(py, "onChunk", None)
        self.signal = get_property(py, "signal", None)

    def to_py(self) raises -> PythonObject:
        var builtins = Python.import_module("builtins")
        var d = builtins.dict()
        d["onChunk"] = self.onChunk
        d["signal"] = self.signal
        return d

struct BashResult(ImplicitlyCopyable):
    var output: String
    var exitCode: PythonObject
    var cancelled: Bool
    var truncated: Bool
    var fullOutputPath: String

    def __init__(out self, output: String, exitCode: PythonObject, cancelled: Bool, truncated: Bool, fullOutputPath: String):
        self.output = output
    ...
    def __init__(out self, *, copy: Self):
        ...
        self.exitCode = copy.exitCode
        self.cancelled = copy.cancelled
        self.truncated = copy.truncated
        self.fullOutputPath = copy.fullOutputPath

    def __init__(out self, py: PythonObject) raises:
        self.output = String(get_property(py, "output", ""))
        self.exitCode = get_property(py, "exitCode", None)
        self.cancelled = Bool(py=get_property(py, "cancelled", False))
        self.truncated = Bool(py=get_property(py, "truncated", False))
        self.fullOutputPath = String(get_property(py, "fullOutputPath", ""))
    ...
    def to_py(self) raises -> PythonObject:
        ...
        d["fullOutputPath"] = self.fullOutputPath
        return d

def executeBashWithOperations(command: PythonObject, cwd: PythonObject, operations: PythonObject, options: PythonObject = None) raises -> PythonObject:
    var builtins = Python.import_module("builtins")
    _init_module()
    var g = _init_globals()
    var code = """
def executeBashWithOperations(command, cwd, operations, options=None):
    command = to_js_obj(command)
    cwd = to_js_obj(cwd)
    operations = to_js_obj(operations)
    options = to_js_obj(options)
    outputChunks = to_py_list()
    outputBytes = 0
    ...
    totalBytes = 0
    def _closure_347():
        nonlocal tempFilePath, tempFileStream
        if tempFilePath:
            return
        id = randomBytes(8).toString("hex")
        tempFilePath = join(tmpdir(), "pi-bash-" + String(id) + ".log")
        tempFileStream = createWriteStream(tempFilePath)
        for chunk in outputChunks:
            tempFileStream.write(chunk)
    ensureTempFile = _closure_347
    decoder = TextDecoder()
    def _closure_539(data):
        nonlocal outputBytes, totalBytes
        data = to_js_obj(data)
        totalBytes = totalBytes + len(data)
        _obj_411 = to_js_obj({"stream": True})
        text = sanitizeBinaryOutput(stripAnsi(String(decoder.decode(data, _obj_411)))).replace(Python.import_module("re").compile("\\r"), "")
        if totalBytes > DEFAULT_MAX_BYTES:
            ensureTempFile()
        if tempFileStream:
            tempFileStream.write(text)
        outputChunks.append(text)
        outputBytes = outputBytes + len(text)
        while (outputBytes > maxOutputBytes and len(outputChunks) > 1):
            removed = outputChunks.shift()
            outputBytes = outputBytes - len(removed)
        if options.onChunk:
            options.onChunk(text)
    onData = _closure_539
    try:
        _obj_571 = to_js_obj({"onData": onData, "signal": options.signal})
        result = wait_promise(operations.exec(command, cwd, _obj_571))
        fullOutput = outputChunks.join("")
        truncationResult = truncateTail(fullOutput)
        if truncationResult.truncated:
            ensureTempFile()
        if tempFileStream:
            tempFileStream.end()
        cancelled = (options.signal.aborted if not objectIs(options.signal.aborted, None) else False)
        _obj_685 = to_js_obj({"output": truncationResult.content if truncationResult.truncated else fullOutput, "exitCode": None if cancelled else ((result.exitCode if not objectIs(result.exitCode, None) else None)), "cancelled": cancelled, "truncated": truncationResult.truncated, "fullOutputPath": tempFilePath})
        return _obj_685
        ...
        if options.signal.aborted:
            ...
            if tempFileStream:
                tempFileStream.end()
            _obj_797 = to_js_obj({"output": truncationResult.content if truncationResult.truncated else fullOutput, "exitCode": None, "cancelled": True, "truncated": truncationResult.truncated, "fullOutputPath": tempFilePath})
            return _obj_797
        if tempFileStream:
            tempFileStream.end()
        raise err
    ...
    try:
        res = g["executeBashWithOperations"](command, cwd, operations, options)
    except e:
        var traceback_mod = Python.import_module("traceback")
        _ = traceback_mod.print_exc()
        raise e^
...
def _init_module() raises:
    var builtins = Python.import_module("builtins")
    var g = _init_globals()
    if builtins.bool(g.get("_module_init_d140749d", False)):
        return
    g["_module_init_d140749d"] = True
    var code = """
...
class BashExecutorOptions:
    def __init__(self, *args, **kwargs):
        if len(args) == 1 and args[0] is not None and not isinstance(args[0], (int, float, str, bool)):
            obj = args[0]
            self.onChunk = obj.get('onChunk') if hasattr(obj, 'get') else getattr(obj, 'onChunk', None)
            self.signal = obj.get('signal') if hasattr(obj, 'get') else getattr(obj, 'signal', None)
        else:
            self.onChunk = args[0] if 0 < len(args) else kwargs.get('onChunk', None)
            self.signal = args[1] if 1 < len(args) else kwargs.get('signal', None)
    def to_py(self):
        return self
...
class BashResult:
    def __init__(self, *args, **kwargs):
        if len(args) == 1 and args[0] is not None and not isinstance(args[0], (int, float, str, bool)):
            ...
            self.exitCode = obj.get('exitCode') if hasattr(obj, 'get') else getattr(obj, 'exitCode', None)
            self.cancelled = obj.get('cancelled') if hasattr(obj, 'get') else getattr(obj, 'cancelled', None)
            self.truncated = obj.get('truncated') if hasattr(obj, 'get') else getattr(obj, 'truncated', None)
            self.fullOutputPath = obj.get('fullOutputPath') if hasattr(obj, 'get') else getattr(obj, 'fullOutputPath', None)
        else:
            self.output = args[0] if 0 < len(args) else kwargs.get('output', None)
            self.exitCode = args[1] if 1 < len(args) else kwargs.get('exitCode', None)
            self.cancelled = args[2] if 2 < len(args) else kwargs.get('cancelled', None)
            self.truncated = args[3] if 3 < len(args) else kwargs.get('truncated', None)
            self.fullOutputPath = args[4] if 4 < len(args) else kwargs.get('fullOutputPath', None)
    def to_py(self):
        return self
