...
from t2m_runtime.utils import Promise, _init_globals, objectIs, to_js_obj, wait_promise
from packages.ai.pi_ai_types import AssistantMessage, OpenRouterRouting, ThinkingBudgets, Usage, VercelGatewayRouting

import packages.ai.pi_ai_types as pi_ai_types



def createAssistantMessageEventStream() raises -> PythonObject:
    _init_module()
...
def _init_module() raises:
    ...
    var g = _init_globals()
    if builtins.bool(g.get("_module_init_9e1a7304", False)):
        return
    g["_module_init_9e1a7304"] = True
...
class EventStream:
    def to_py(self):
        return self
    def __init__(self, isComplete, extractResult):
        self = to_js_obj(self)
        isComplete = to_js_obj(isComplete)
        extractResult = to_js_obj(extractResult)
        self.queue = JSList([])
        self.waiting = JSList([])
        self.done = False
        self.isComplete = isComplete
        self.extractResult = extractResult
        def _closure_205(resolve):
            resolve = to_js_obj(resolve)
            self.resolveFinalResult = resolve
        self.finalResultPromise = Promise(_closure_205)
    def push(self, event):
        self = to_js_obj(self)
        event = to_js_obj(event)
        if self.done:
            return
        if self.isComplete(event):
            self.done = True
            self.resolveFinalResult(self.extractResult(event))
        waiter = self.waiting.shift()
        if waiter:
            _obj_312 = to_js_obj({"value": event, "done": False})
            waiter(_obj_312)
        else:
    ...
    def end(self, result=None):
        self = to_js_obj(self)
        result = to_js_obj(result)
        self.done = True
        if result != None:
            self.resolveFinalResult(result)
        while (len(self.waiting) > 0):
            waiter = self.waiting.shift()
            _obj_426 = to_js_obj({"value": None, "done": True})
            waiter(_obj_426)
    def __aiter__(self):
        self = to_js_obj(self)
        while (True):
            if len(self.queue) > 0:
                ...
                def _closure_546(resolve):
                    resolve = to_js_obj(resolve)
                    return self.waiting.append(resolve)
                result = wait_promise(Promise(_closure_546))
                if result.done:
                    return
                yield result.value
    def __iter__(self):
    ...
    def result(self):
        self = to_js_obj(self)
        return self.finalResultPromise
...
class AssistantMessageEventStream(EventStream):
    ...
    def __init__(self):
        self = to_js_obj(self)
        def _closure_639(event):
            event = to_js_obj(event)
            return event.type == "done" or event.type == "error"
        def _closure_704(event):
            event = to_js_obj(event)
            if event.type == "done":
                return event.message
            elif event.type == "error":
                return event.error
            raise Error("Unexpected event type for final result")
        super().__init__(_closure_639, _closure_704)
...
def EventStream(isComplete: PythonObject, extractResult: PythonObject) raises -> PythonObject:
    _init_module()
    var g = _init_globals()
    return g["EventStream"](isComplete, extractResult)

def AssistantMessageEventStream() raises -> PythonObject:
    _init_module()
    var g = _init_globals()
    return g["AssistantMessageEventStream"]()
