...
from packages.ai.pi_ai_registry import getApiProvider
from packages.ai.pi_ai_types import AssistantMessage, AssistantMessageEventStream, Context, Model, OpenRouterRouting, SimpleStreamOptions, StreamOptions, ThinkingBudgets, Usage, VercelGatewayRouting

import packages.ai.pi_ai_registry as pi_ai_registry
import packages.ai.pi_ai_types as pi_ai_types

def resolveApiProvider(api: PythonObject) raises -> PythonObject:
    _init_module()
    var provider: PythonObject = getApiProvider(api)
    if not provider:
        raise Error("No API provider registered for api: " + String(api))
    return provider

def stream(model: PythonObject, context: Context, options: PythonObject = None) raises -> PythonObject:
    _init_module()
    var provider: PythonObject = resolveApiProvider(model.api)
    return provider.stream(model, context.to_py(), options)

def complete(model: PythonObject, context: Context, options: PythonObject = None) raises -> PythonObject:
    _init_module()
    var s: PythonObject = stream(model, context, options)
    return s.result()

def streamSimple(model: PythonObject, context: Context, options: PythonObject = None) raises -> PythonObject:
    _init_module()
    var provider: PythonObject = resolveApiProvider(model.api)
    return provider.streamSimple(model, context.to_py(), options)

def completeSimple(model: PythonObject, context: Context, options: PythonObject = None) raises -> PythonObject:
    _init_module()
    var s: PythonObject = streamSimple(model, context, options)
    return s.result()

def _init_module() raises:
    var builtins = Python.import_module("builtins")
    var g = _init_globals()
    if builtins.bool(g.get("_module_init_96c11692", False)):
        return
    g["_module_init_96c11692"] = True
...
def stream(model, context, options=None):
    ...
    provider = resolveApiProvider(model.api)
    return provider.stream(model, context.to_py(), options)

def complete(model, context, options=None):
    model = to_js_obj(model)
    context = to_js_obj(context)
    options = to_js_obj(options)
    s = stream(model, context, options)
...
def streamSimple(model, context, options=None):
    model = to_js_obj(model)
    context = to_js_obj(context)
    options = to_js_obj(options)
    provider = resolveApiProvider(model.api)
...
def completeSimple(model, context, options=None):
    model = to_js_obj(model)
    context = to_js_obj(context)
    options = to_js_obj(options)
    s = streamSimple(model, context, options)
