...
def call_gemini_with_tool_response(
    api_key: String,
    prompt: String,
    model_turn: PythonObject,
    func_name: String,
    tool_response: String,
    tools_def: PythonObject
) raises -> String:
    var json = Python.import_module("json")
    var urllib = Python.import_module("urllib.request")
    ...
    var contents = json.loads('[{"role": "user", "parts": [{"text": ""}]}]')
    contents[0]["parts"][0]["text"] = prompt
    
    # 2. Append original model turn
    _ = contents.append(model_turn)
    
    # 3. Create function response turn
    var function_part = json.loads('{"role": "user", "parts": [{"functionResponse": {"name": "", "response": {}}}]}')
    function_part["parts"][0]["functionResponse"]["name"] = func_name
    
    # Propagate function call ID if present
    var first_part = model_turn["parts"][0]
    if "functionCall" in first_part and "id" in first_part["functionCall"]:
        function_part["parts"][0]["functionResponse"]["id"] = first_part["functionCall"]["id"]
        
    var response_dict = json.loads('{"output": {}}')
    response_dict["output"] = json.loads(tool_response)
    function_part["parts"][0]["functionResponse"]["response"] = response_dict
    
    _ = contents.append(function_part)
    
    var second_body = json.loads('{}')
    second_body["contents"] = contents
    second_body["tools"] = tools_def
